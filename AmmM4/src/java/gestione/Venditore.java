...
public class Venditore extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (session.getAttribute("vLoggato").equals(true)) {   ///si occupa di ri-indirizzare il venditore alla pagina venditore.jsp
            request.getRequestDispatcher("M3/venditore.jsp").forward(request, response);
        } else {
            request.getRequestDispatcher("M3/errore.jsp");
        }

        if (request.getParameter("submit_venditore") != null) {  //se si "inserisce" un libro, i dati di questo libro vengono portati a una pagine di conferma
            ...
            request.getRequestDispatcher("conferma_venditore.jsp").forward(request, response);
        }
    }
